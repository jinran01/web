<template>
    <div class="changeemail-box">
        <Navbar v-slot:default>手机绑定</Navbar>
...
import {getPhoneCode,BindPhone} from "@/network/profile";
    ...
    setup(){
        ...
        const  init = ()=>{
            setInterval(()=>{
                ...
                if (time - window.localStorage.getItem('sendphonetime') < 60*1000){
                    state.flag_btn = true;
                }
                if(time - window.localStorage.getItem('sendphonetime') > 60*1000){
            ...
            if (phone.value){
                ...
                getPhoneCode(params).then(res=>{
                    if (res.status == '204'){
                        ...
                        window.localStorage.setItem('sendphonetime',time)
                        setInterval(()=>{
                ...
                Notify({message:'手机号有误',position:'center'})
            }
        }
